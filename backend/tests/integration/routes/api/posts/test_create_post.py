from support.factories import post_payload
from support.json import pick


def test_create_post_returns_created_post_for_authenticated_user(
    test_client,
    auth_headers,
):
...
def test_create_post_returns_unprocessable_content_for_invalid_data(
...
def test_create_post_returns_unprocessable_content_when_no_params_were_used(
    test_client, auth_headers
):
    response = test_client.post(
        "/api/posts",
...
def test_create_post_translates_validation_errors_from_accept_language(
    test_client, auth_headers
):
...
def test_create_post_falls_back_to_english_for_an_unknown_locale(
    test_client, auth_headers
):
...
def test_create_post_returns_unprocessable_content_for_business_logic_validations(
    test_client, auth_headers, post_factory
):
    existing_record = post_factory(user_uid="current_user", title="Existing title")
...
def test_create_post_requires_authentication(test_client):
